      ...
      <el-table :data="orders" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" fixed="right" width="240">
          <template #default="{ row }">
            ...
            <el-button v-if="canDelete(row)" link type="danger" @click="deleteOrder(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
const deletableStatuses = [0, 2, 8, 10]
const canDelete = (order) => order && deletableStatuses.includes(order.status)
...
const deleteOrder = async (order) => {
  try {
    await ElMessageBox.confirm('确认删除该订单？删除后不可恢复', '确认', { type: 'warning' })
    const res = await api.admin.orders.delete(order.id)
    if (res.code === 200) {
    ...
    if (error !== 'cancel') console.error('Failed to delete order:', error)
  }
}
...
const exportOrders = async () => {
  try {
    ...
    // request as blob
    const res = await api.admin.orders.export(params)
    // If axios responseType is 'blob', res will be a Blob object
    const blob = new Blob([res], { type: 'text/csv;charset=utf-8' })
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    const filename = `orders_export_${dayjs().format('YYYYMMDD')}.csv`
    a.href = url
    a.download = filename
