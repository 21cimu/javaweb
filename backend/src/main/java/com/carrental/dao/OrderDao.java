...
public class OrderDao extends BaseDao<Order> {
    ...
    protected Order mapRow(ResultSet rs) throws SQLException {
        ...
        order.setSpendingAccounted(getInt(rs, "spending_accounted"));
        return order;
    }
    ...
    /**
     * Refund deposit (record refund_amount)
     * This will increase orders.refund_amount but ensure refund_amount does not exceed paid_amount,
     * preventing (paid_amount - refund_amount) from becoming negative in reports.
     */
    public int refundDeposit(Long id, java.math.BigDecimal amount) {
        String sql = "UPDATE orders SET refund_amount = LEAST(COALESCE(refund_amount, 0) + ?, COALESCE(paid_amount, 0)), updated_at = NOW() WHERE id = ?";
    ...
    public java.util.Map<String, Object> getTodayStats() {
        ...
        // Today's GMV (only count completed orders and subtract refunds)
        String gmvSql = "SELECT COALESCE(SUM(COALESCE(paid_amount,0) - COALESCE(refund_amount,0)), 0) as gmv " +
                "FROM orders WHERE DATE(created_at) = CURDATE() AND status = 8";
        stats.put("gmv", executeCount(gmvSql));
    ...
    /**
     * Count orders by vehicle id
     */
    public long countByVehicleId(Long vehicleId) {
        String sql = "SELECT COUNT(*) FROM orders WHERE vehicle_id = ?";
        return executeCount(sql, vehicleId);
    }

    /**
     * Find reviews with optional filters (rating, vehicleId, userId) with pagination
     */
    public List<Order> findReviews(Integer rating, Long vehicleId, Long userId, int page, int pageSize) {
        StringBuilder sql = new StringBuilder("SELECT * FROM orders WHERE review IS NOT NULL AND review <> ''");
        List<Object> params = new ArrayList<>();
        ...
        sql.append(" ORDER BY review_time DESC LIMIT ? OFFSET ?");
        params.add(pageSize);
        params.add((page - 1) * pageSize);
     ...
     */
    public long countReviews(Integer rating, Long vehicleId, Long userId) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM orders WHERE review IS NOT NULL AND review <> ''");
        List<Object> params = new ArrayList<>();
    ...
    /**
     * Clear review fields for an order (soft delete of comment)
     */
    public int clearReview(Long orderId) {
        String sql = "UPDATE orders SET rating = NULL, review = NULL, review_images = NULL, review_time = NULL, updated_at = NOW() WHERE id = ?";
        return executeUpdate(sql, orderId);
    }

    /**
     * Mark order as having its spending accounted (set spending_accounted = 1)
     */
    public int markSpendingAccounted(Long id) {
        String sql = "UPDATE orders SET spending_accounted = 1, updated_at = NOW() WHERE id = ?";
        return executeUpdate(sql, id);
    }
}
