...
public class VehicleDao extends BaseDao<Vehicle> {
    ...
    protected Vehicle mapRow(ResultSet rs) throws SQLException {
        ...
        vehicle.setLastMaintenanceDate(getLocalDate(rs, "last_maintenance_date"));
    ...
    public Long create(Vehicle vehicle) {
        String sql = """
            INSERT INTO vehicles (vin, plate_number, brand, model, series, year, color,
                ...
                last_maintenance_date, view_count, order_count, rating, is_hot, is_new, no_deposit,
        ...
        return executeInsert(sql, vehicle.getVin(), vehicle.getPlateNumber(),
            ...
            vehicle.getLastMaintenanceDate(), vehicle.getViewCount(),
            vehicle.getOrderCount(), vehicle.getRating(), vehicle.getIsHot(),
            vehicle.getIsNew(), vehicle.getNoDeposit());
    }
    ...
    public int update(Vehicle vehicle) {
        String sql = """
            UPDATE vehicles SET 
                ...
                last_maintenance_date = ?, is_hot = ?, is_new = ?, no_deposit = ?, updated_at = NOW()
            WHERE id = ?
            """;
        return executeUpdate(sql, vehicle.getVin(), vehicle.getPlateNumber(),
            ...
            vehicle.getLastMaintenanceDate(), vehicle.getIsHot(),
            vehicle.getIsNew(), vehicle.getNoDeposit(), vehicle.getId());
    }
    ...
    public int updateStatus(Long id, Integer status) {
        ...
        Integer fromStatus = current != null ? current.getStatus() : null;

        String sql = "UPDATE vehicles SET status = ?, updated_at = NOW() WHERE id = ?";
        int result = executeUpdate(sql, status, id);

        if (result > 0 && (fromStatus == null || !fromStatus.equals(status))) {
            try {
                VehicleStatusLog log = new VehicleStatusLog();
                log.setVehicleId(id);
                if (current != null) {
                    log.setVehicleName(current.getBrand() + " " + current.getModel());
                    log.setPlateNumber(current.getPlateNumber());
                }
                log.setFromStatus(fromStatus);
                log.setToStatus(status);
                log.setOperatorId(LogContext.getUserId());
                log.setOperatorName(LogContext.getUsername());
                log.setOperatorRole(LogContext.getRole());
                log.setRemark(null);
                vehicleStatusLogDao.create(log);
    ...
    /**
     * Find all vehicles for admin with pagination
     */
    ...
    /**
     * Find vehicles by status for admin with pagination
     */
    public List<Vehicle> findByStatus(Integer status, int page, int pageSize) {
        String sql = """
            ...
            LIMIT ? OFFSET ?
            """;
        return executeQuery(sql, status, pageSize, (page - 1) * pageSize);
    }
    ...
    /**
     * Find vehicles by store with pagination
     */
    ...
    /**
     * Count vehicles by store
     */
    public long countByStore(Long storeId) {
        String sql = "SELECT COUNT(*) FROM vehicles WHERE store_id = ?";
        return executeCount(sql, storeId);
    }

    /**
     * Find vehicles by store and status with pagination
     */
    public List<Vehicle> findByStoreAndStatus(Long storeId, Integer status, int page, int pageSize) {
        String sql = """
            ...
            LIMIT ? OFFSET ?
            """;
        return executeQuery(sql, storeId, status, pageSize, (page - 1) * pageSize);
    }

    /**
     * Count vehicles by store and status
     */
    public long countByStoreAndStatus(Long storeId, Integer status) {
        String sql = "SELECT COUNT(*) FROM vehicles WHERE store_id = ? AND status = ?";
        return executeCount(sql, storeId, status);
    }

    /**
     * Physically delete a vehicle by id
     */
